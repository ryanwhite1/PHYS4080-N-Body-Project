...
//  in_real(instr, &tnow);                      /* read starting time       */
    ...
    nbody=0;
    while (fgets(inlin,250,instr) != NULL) nbody++;
    fprintf(stderr,"Found %i particles !\n",nbody);
    fclose(instr); 
    ...
    for (p = bodytab; p < bodytab+nbody; p++) {
       fscanf(instr, "%f %f %f %f %f %f %f", &Mass(p), &Pos(p)[0], &Pos(p)[1], &Pos(p)[2], &Vel(p)[0], &Vel(p)[1], &Vel(p)[2]);
    }
                                                /* allocate body array      */
...
//  sprintf(namebuf, outfile, nstep);           /* construct output name    */
    if (tnow==0.0) {
      strcpy(namebuf,"mkdir ");
      strcat(namebuf,outfile);
      strcat(namebuf," 2>/dev/null");
      system(namebuf);

      strcpy(namebuf,"rm ");
      strcat(namebuf,outfile);
      strcat(namebuf,"/snap* 2>/dev/null");
      system(namebuf);
    }
    sprintf(timestr,"%07.3f",tnow);
    strcpy (namebuf,outfile);
    strcat (namebuf,"/snap_t");
    strcat (namebuf,timestr);
    strcat (namebuf,".dat");
...
//          out_vector(outstr, Acc(p));         /* output accelerations     */
    for (p = bodytab; p < bodytab+nbody; p++) 
       fprintf(outstr, "%15.8e %15.8e %15.8e %15.8e %15.8e %15.8e %15.8e\n", Mass(p), Pos(p)[0], Pos(p)[1], Pos(p)[2], Vel(p)[0], Vel(p)[1], Vel(p)[2]);
